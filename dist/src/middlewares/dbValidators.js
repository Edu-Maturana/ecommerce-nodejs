"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.brandExists = exports.isAdmin = exports.userExists = void 0;
const user_1 = __importDefault(require("../models/user"));
const brand_1 = __importDefault(require("../models/brand"));
const config_1 = __importDefault(require("../../config"));
const userExists = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { email } = req.body;
    const user = yield user_1.default.findOne({ where: { email } });
    if (user) {
        return res.status(400).json({ message: "User already exists" });
    }
    next();
});
exports.userExists = userExists;
const isAdmin = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.user;
    const user = yield user_1.default.findOne({ where: { id } });
    if (!user) {
        return res.status(400).json({ message: "User not found" });
    }
    if (user.email !== config_1.default.ADMIN_EMAIL) {
        return res.status(401).json({ message: "You are not authorized" });
    }
    next();
});
exports.isAdmin = isAdmin;
const brandExists = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { brand } = req.body;
    const exists = yield brand_1.default.findOne({ where: {
            name: brand,
        } });
    if (!exists) {
        return res.status(400).json({ message: "Brand does not exist" });
    }
    next();
});
exports.brandExists = brandExists;
//# sourceMappingURL=dbValidators.js.map