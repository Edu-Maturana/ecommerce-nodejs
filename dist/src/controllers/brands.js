"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.editBrand = exports.getBrands = exports.createBrand = void 0;
const brand_1 = __importDefault(require("../models/brand"));
const createBrand = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { name } = req.body;
    const brand = yield brand_1.default.create({ name });
    return res.status(200).json({
        message: "Brand created successfully",
        brand,
    });
});
exports.createBrand = createBrand;
const getBrands = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const brands = yield brand_1.default.findAll({
        attributes: ["id", "name"],
    });
    return res.status(200).json({
        brands,
    });
});
exports.getBrands = getBrands;
const editBrand = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const { name } = req.body;
    const brand = yield brand_1.default.findByPk(id);
    if (!brand) {
        return res.status(400).json({ message: "Brand not found" });
    }
    yield brand.update({ name });
    return res.status(200).json({
        message: "Brand updated successfully",
    });
});
exports.editBrand = editBrand;
//# sourceMappingURL=brands.js.map